---
title: "Study 1 - Relationship - binomial GLMM - power simulation"
...
fam.per <- c(0.58) # performance in family dog group
ass.per <- c(0.8) # performance in assistant dog group
she.per <- c(0.3) # performance in shelter dog group

# add condition and trial number
start.data <- expand.grid(trial=c(1:4), condition = c("fam", "ass", "she"))
# Repeat All Rows
...
# relevel condition
start.data$condition <- relevel(as.factor(start.data$condition), "fam", "ass", "she")
...
all.res$icpt <- NA
all.res$conditionass <- NA
all.res$conditionshe <- NA
all.res$re.sd <- NA
all.res$warns.full <- NA
all.res$warns.null <- NA
all.res$lrt.p.con <- NA
all.res$full.null.p <- NA
all.res$conditionass.p <- NA
all.res$conditionshe.p <- NA
...
for (i in 1:nrow(all.res)) {
  ...
  coefs <- c(
  "(Intercept)" = log(all.res[i, "fam.per"] / (1 - all.res[i, "fam.per"])),
  "conditionass" = log(all.res[i, "ass.per"] / (1 - all.res[i, "ass.per"] )) - log(all.res[i, "fam.per"] / (1 - all.res[i, "fam.per"])),
  "conditionshe" = log(all.res[i, "she.per"] / (1 - all.res[i, "she.per"] )) - log(all.res[i, "fam.per"] / (1 - all.res[i, "fam.per"])),
  "z.age" = 0,
  "sexm" = 0,
  "z.trial" = 0
)
  ...
  # store results:
  all.res[i, c("icpt", "conditionass", "conditionshe", "z.age", "sexm", "z.trial")] <- fixef(full$value)
  ...
  all.res[i, "conditionass.p"] <- as.data.frame(summary(full$value)$coef)["conditionass", "Pr(>|z|)"]
  all.res[i, "conditionshe.p"] <- as.data.frame(summary(full$value)$coef)["conditionshe", "Pr(>|z|)"]
  print(i)
}

summary(full$value)
save.image("power_sim_study1_N60.RData")
```
...
```{r echo=FALSE}
#null model: 
tapply(X=all.res[, "warns.null"]>0, INDEX=all.res[, c("r.effect")],
FUN=sum)
```
...
plot(
  ...
  y = unlist(all.res[, c("icpt", "conditionass", "conditionshe")]),
  pch = 19, col = grey(level = 0.2, alpha = 0.2),
  xaxt = "n", xlim = c(0.5, 3.5), ylab = "estimate", xlab = ""
)
mtext(text = c("icpt", "conditionass", "conditionshe"), side = 1, at = 1:3, line = 0.2)
```
...
lrt.data2 <- all.res2 %>%
  ...
  group_by(fam.per, ass.per, she.per, r.effect) %>%
  summarise(condition_ass=mean(conditionass),
            condition_she=mean(conditionshe),
            z.age=mean(z.age),
            sex=mean(sexm),
            ...
            conditionass.p.sign.prop = length(conditionass.p[conditionass.p < 0.05]) / n.simus,
            conditionshe.p.sign.prop = length(conditionshe.p[conditionshe.p < 0.05]) / n.simus)
